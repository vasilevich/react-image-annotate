...
export const MainLayout = ({
    ...
    const key = `${JSON.stringify(state)}_${new Date().getDate()}`;
    return (
        <FullScreenContainer>
            <FullScreen
                    ...
                    <Workspace
                        ...
                        headerLeftSide={[
                            state.annotationType === "video" ? (
                                ...
                                <div key={`${key}_1`} className={classes.headerTitle}>{activeImage.name}</div>
                            ) : null,
                        ].filter(Boolean)}
                        headerItems={[
                            !hidePrev && {name: "Prev"},
                        ...
                        rightSidebarItems={[
                            debugModeOn && (
                                <DebugBox key={`${key}_1`} state={debugModeOn} lastAction={state.lastAction}/>
                            ),
                            state.taskDescription && (
                                <TaskDescription key={`${key}_2`} description={state.taskDescription}/>
                            ),
                            state.regionClsList && (
                                <ClassSelectionMenu
                                    key={`${key}_3`}
